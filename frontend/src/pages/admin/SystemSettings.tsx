...
export default function SystemSettings() {
  // API Settings state
  const [ibmApiKey, setIbmApiKey] = useState('ibm_api_key_••••••••••••••••')
  const [modelId, setModelId] = useState('ibm/granite-13b-instruct-v2')
  const [projectId, setProjectId] = useState('proj_saathi_ai_001')
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader className="pb-3">
          <CardTitle className="text-sm flex items-center gap-2">
            ...
            API Settings — IBM watsonx.ai
          </CardTitle>
          <CardDescription>watsonx.ai connection configuration</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="flex items-center gap-3 mb-1">
            ...
            {connStatus === 'idle' && (
              <span className="flex items-center gap-1 text-xs font-semibold text-gray-500 bg-gray-50 border border-gray-200 rounded-full px-2.5 py-0.5">
                Not tested
              </span>
            )}
          </div>
          ...
          <div>
            <Label className="text-xs font-medium text-gray-700 mb-1">IBM API Key</Label>
            <Input
              type="password"
              value={ibmApiKey}
              onChange={(e) => setIbmApiKey(e.target.value)}
              placeholder="ibm_api_key_…"
              autoComplete="off"
            />
          </div>
          <div>
            <Label className="text-xs font-medium text-gray-700 mb-1">Model ID</Label>
            <Input
              value={modelId}
              onChange={(e) => setModelId(e.target.value)}
              placeholder="ibm/granite-13b-instruct-v2"
            />
          </div>
          <div>
            <Label className="text-xs font-medium text-gray-700 mb-1">Project ID</Label>
            <Input
              value={projectId}
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <Toggle
            ...
            label="Mock OTP Mode (use '123456' for all logins — disable in production)"
          />
          <div>
            <Label className="text-xs font-medium text-gray-700 mb-1">OTP Expiry (seconds)</Label>
            <Input
              type="number"
              min={60}
              max={900}
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div>
            <Label className="text-xs font-medium text-gray-700 mb-1">App Name</Label>
            <Input value={appName} onChange={(e) => setAppName(e.target.value)} className="max-w-80" />
          </div>
          <Toggle
            checked={debugMode}
            onChange={setDebugMode}
            label="Debug Mode (verbose logging to console)"
          />
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div>
            <Label className="text-xs font-medium text-gray-700 mb-1">Session Timeout (minutes)</Label>
            <Input type="number" defaultValue="60" className="max-w-40" />
          </div>
          <div>
            <Label className="text-xs font-medium text-gray-700 mb-1">Max Failed Login Attempts</Label>
            <Input type="number" defaultValue="5" className="max-w-40" />
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader className="pb-3">
          ...
          <CardDescription>Status of connected external services</CardDescription>
        </CardHeader>
        <CardContent className="space-y-3">
          {[
            { name: 'IBM watsonx.ai',       status: 'connected', endpoint: 'us-south.ml.cloud.ibm.com' },
            { name: 'e-Shram API',           status: 'connected', endpoint: 'api.eshram.gov.in'         },
            { name: 'UIDAI Aadhaar Verify',  status: 'pending',   endpoint: 'stage1.uidai.gov.in'       },
